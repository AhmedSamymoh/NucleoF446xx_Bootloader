...
def process_COMMAND_BL_GET_VER(length):
    ...
    value = bytearray(ver)
    print("\n   Bootloader SW Vendor ID : ",hex(value[0]))
    print("   Bootloader SW Major Version : ",hex(value[1]))
    print("   Bootloader SW Minor Version : ",hex(value[2]))
    print("   Bootloader SW Patch Version : ",hex(value[3]))
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        ...
        go_address  = input("\n   Please enter 4 bytes go address in hex: 0x")
        ...
        if(sector_num != 0xFF):
            ...
            # Default value when no sectors are specified
            data_buf[3] = 0  # Or any other valid value representing no sectors
        ...
        ret_value = read_bootloader_reply    
        ...
        base_mem_address = input("\n   Enter the memory write address here : 0x")
        ...
        print( "\n\n"
            "  **************** Firmware Updated !! ****************\n"
            "  Bytes Sent: {0} -- Bytes Remaining: {1}\n"
            .format(bytes_so_far_sent, bytes_remaining)
        )    
        mem_write_active=0
